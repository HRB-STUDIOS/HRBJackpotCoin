...
#ifndef HRBJACKPOTCOIN_VALIDATION_H
#define HRBJACKPOTCOIN_VALIDATION_H
...
// HRB Jackpot Coin: Structure to return detailed block reward distribution
struct BlockReward {
    CAmount miner_reward;
    CAmount founder_reimbursement;
    CAmount mini_jackpot;
    CAmount minor_jackpot;
    CAmount major_jackpot;
    CAmount grand_jackpot;

    // HRB Jackpot Coin: Operator to allow implicit conversion to CAmount
    // This makes BlockReward usable in contexts expecting a single CAmount (e.g., for money supply tracking, or some test cases).
    // It sums all components, representing the total value minted by the subsidy.
    operator CAmount() const {
        return miner_reward + founder_reimbursement + mini_jackpot + minor_jackpot + major_jackpot + grand_jackpot;
    }

    // HRB Jackpot Coin: Define operator+ for CAmount + BlockReward (for scenarios like nFees + GetBlockSubsidy(...))
    // This allows direct addition of a CAmount to a BlockReward object.
    friend CAmount operator+(CAmount lhs, const BlockReward& rhs) {
        return lhs + static_cast<CAmount>(rhs); // Uses the operator CAmount() defined above
    }
    // Also define for BlockReward + CAmount, for completeness.
    friend CAmount operator+(const BlockReward& lhs, CAmount rhs) {
        return static_cast<CAmount>(lhs) + rhs; // Uses the operator CAmount() defined above
    }

    // HRB Jackpot Coin: Define stream operator for BlockReward for logging and Boost.Test
    // This will fix 'no match for operator<<' errors when Boost.Test or LogPrintf tries to print BlockReward objects.
    // It provides a human-readable representation of the struct's contents.
    // This is crucial for test files (like validation_tests.cpp) and any logging statements.
    friend std::ostream& operator<<(std::ostream& os, const BlockReward& br) {
        os << "{miner_reward=" << br.miner_reward
           << ", founder_reimbursement=" << br.founder_reimbursement
           << ", mini_jackpot=" << br.mini_jackpot
           << ", minor_jackpot=" << br.minor_jackpot
           << ", major_jackpot=" << br.major_jackpot
           << ", grand_jackpot=" << br.grand_jackpot
           << "}";
        return os;
    }
};
...
BlockReward GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams);
...
#endif // HRBJACKPOTCOIN_VALIDATION_H
