...
UniValue blockToJSON(BlockManager& blockman, const CBlock& block, const CBlockIndex* tip, const CBlockIndex* blockindex, TxVerbosity verbosity)
{
    ...
    if (blockindex) {
        result.pushKV("moneysupply", blockindex->nMoneySupply / COIN);
    }
        ...
        {RPCResult::Type::OBJ, "", "",
        {
            ...
            {RPCResult::Type::OBJ, "prevout", "(Only if undo information is available)",
            {
                ...
                {RPCResult::Type::OBJ, "scriptPubKey", "",
                {
                    ...
                    {RPCResult::Type::STR, "address", /*optional=*/true, "The HRBJackpotCoin address (only if a well-defined address exists)"},
...
static RPCHelpMan getblockstats()
{
    return RPCHelpMan{"getblockstats",
                "\nCompute per block statistics for a given window. All amounts are in hrbjacks.\n"
            ...
            RPCResult::Type::OBJ, "", "",
            {
                ...
                {RPCResult::Type::NUM, "avgfeerate", /*optional=*/true, "Average feerate (in hrbjacks per virtual byte)"},
                {RPCResult::Type::NUM, "avgtxsize", /*optional=*/true, "Average transaction size"},
                {RPCResult::Type::STR_HEX, "blockhash", /*optional=*/true, "The block hash (to check for potential reorgs)"},
                {RPCResult::Type::ARR_FIXED, "feerate_percentiles", /*optional=*/true, "Feerates at the 10th, 25th, 50th, 75th, and 90th percentile weight unit (in hrbjacks per virtual byte)",
                {
                ...
                {RPCResult::Type::NUM, "maxfeerate", /*optional=*/true, "Maximum feerate (in hrbjacks per virtual byte)"},
                {RPCResult::Type::NUM, "maxtxsize", /*optional=*/true, "Maximum transaction size"},
                {RPCResult::Type::NUM, "medianfee", /*optional=*/true, "Truncated median fee in the block"},
                {RPCResult::Type::NUM, "mediantime", /*optional=*/true, "The block median time past"},
                {RPCResult::Type::NUM, "mediantxsize", /*optional=*/true, "Truncated median transaction size"},
                {RPCResult::Type::NUM, "minfee", /*optional=*/true, "Minimum fee in the block"},
                {RPCResult::Type::NUM, "minfeerate", /*optional=*/true, "Minimum feerate (in hrbjacks per virtual byte)"},
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    for (size_t i = 0; i < block.vtx.size(); ++i) {
        ...
        if (loop_inputs) {
            ...
            // New feerate uses hrbjacks per virtual byte instead of per serialized byte
    ...
    // HRB Jackpot Coin: Get the BlockReward struct with detailed breakdown
    BlockReward block_reward = GetBlockSubsidy(pindex.nHeight, chainman.GetParams().GetConsensus());

    // Create a UniValue object to hold the detailed reward breakdown for RPC output
    UniValue subsidy_obj(UniValue::VOBJ);
    subsidy_obj.pushKV("miner_reward", FormatMoney(block_reward.miner_reward));
    subsidy_obj.pushKV("founder_reimbursement", FormatMoney(block_reward.founder_reimbursement));
    subsidy_obj.pushKV("mini_jackpot", FormatMoney(block_reward.mini_jackpot));
    subsidy_obj.pushKV("minor_jackpot", FormatMoney(block_reward.minor_jackpot));
    subsidy_obj.pushKV("major_jackpot", FormatMoney(block_reward.major_jackpot));
    subsidy_obj.pushKV("grand_jackpot", FormatMoney(block_reward.grand_jackpot));
    // Include total_minted using the implicit CAmount conversion for completeness
    subsidy_obj.pushKV("total_minted", FormatMoney(static_cast<CAmount>(block_reward)));

    // Add the detailed UniValue object for "subsidy"
    ret_all.pushKV("subsidy", subsidy_obj);
...
static RPCHelpMan scanblocks()
{
    return RPCHelpMan{"scanblocks",
        ...
        "This call may take several minutes. Make sure to use no RPC timeout (hrbjackpotcoin-cli -rpcclienttimeout=0)",
        {
            scan_action_arg_desc,
            scan_objects_arg_desc,
    ...
    const fs::path& temppath)
{
...

    {
        ...
        //
        // See discussion here:
        //   https://github.com/hrbjackpotcoin/hrbjackpotcoin/pull/15606#discussion_r274479369
        //
        LOCK(::cs_main);
...
static RPCHelpMan loadtxoutset()
{
    return RPCHelpMan{
        ...
        "The result is a usable hrbjackpotcoind instance that is current with the network tip in a "
        ...
        "document (<https://github.com/hrbjackpotcoin/hrbjackpotcoin/blob/master/doc/design/assumeutxo.md>).",
        {
            {"path",
                RPCArg::Type::STR,
                RPCArg::Optional::NO,
