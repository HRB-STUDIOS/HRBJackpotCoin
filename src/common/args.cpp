...
const char * const HRBJACKPOTCOIN_CONF_FILENAME = "hrbjackpotcoin.conf";
const char * const HRBJACKPOTCOIN_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //hrbjackpotcoin-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", HRBJACKPOTCOIN_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\HRBJackpotCoin
    // macOS: ~/Library/Application Support/HRBJackpotCoin
    // Unix-like: ~/.hrbjackpotcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "HRBJackpotCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/HRBJackpotCoin";
#else
    // Unix-like
    return pathRet / ".hrbjackpotcoin";
#endif
#endif
}
