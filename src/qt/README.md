This directory contains the source code for the HRBJackpotCoin Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
#### hrbjackpotcoingui.(h/cpp)

- Represents the main window of the HRBJackpotCoin UI.
...
- ClientModel is used by the main application `hrbjackpotcoingui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a hrbjackpotcoin:... link from a browser).
...
* UI elements like HRBJackpotCoinAmountField, which inherit from QWidget.
* `hrbjackpotcoinstrings.cpp`: automatically generated
* `hrbjackpotcoinunits.(h/cpp)`: hrb / mhrb / etc. handling
* `callback.h`
...
**Note:** Do not change `local/hrbjackpotcoin_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `hrbjackpotcoin-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `hrbjackpotcoin-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `hrbjackpotcoin-qt`
9. You're all set! Start developing, building, and debugging the HRBJackpotCoin Core GUI
