...
WalletModel* WalletController::createWalletSilently(const std::string& name,
...
{
#ifndef USE_SQLITE
    error = _("Cannot create new wallet, the software was compiled without sqlite support (required for descriptor wallets)");
    return nullptr;
#endif

    auto result = m_node.walletLoader().createWallet(name, passphrase, wallet_creation_flags, warnings);
    if (!result) {
        error = util::ErrorString(result);   // <-- use ErrorString
        return nullptr;
    }

    std::unique_ptr<interfaces::Wallet> wallet = std::move(*result);
    WalletModel* model = getOrCreateWallet(std::move(wallet));
    Q_EMIT walletAdded(model);
    return model;
}
