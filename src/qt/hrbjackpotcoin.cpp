...
#include <qt/jackpotviewdialog.h> // ADDED: New dialog for jackpot view
...
#include <util/strencodings.h> // For fs::quoted
#include <util/string.h> // For MakeUnorderedList
...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<HRBJackpotCoinUnit>("HRBJackpotCoinUnit");
#else
    qRegisterMetaType<HRBJackpotCoinUnit>("HRBJackpotCoinUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. hrbjackpotcoin_de.qm (shortcut "de" needs to be defined in hrbjackpotcoin.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. hrbjackpotcoin_de_DE.qm (shortcut "de_DE" needs to be defined in hrbjackpotcoin.qrc)
...
static const char* qt_argv = "hrbjackpotcoin-qt";

HRBJackpotCoinApplication::HRBJackpotCoinApplication()
...
void HRBJackpotCoinApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the HRBJackpotCoinApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", HRBJackpotCoinGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void HRBJackpotCoinApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool HRBJackpotCoinApplication::createOptionsModel(bool resetSettings)
{
...
void HRBJackpotCoinApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new HRBJackpotCoinGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &HRBJackpotCoinGUI::quitRequested, this, &HRBJackpotCoinApplication::requestShutdown);
...
void HRBJackpotCoinApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void HRBJackpotCoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool HRBJackpotCoinApplication::baseInitialize()
{
    return node().baseInitialize();
}

void HRBJackpotCoinApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /* communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &HRBJackpotCoinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &HRBJackpotCoinApplication::handleRunawayException);
    connect(this, &HRBJackpotCoinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &HRBJackpotCoinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void HRBJackpotCoinApplication::parameterSetup()
{
...
void HRBJackpotCoinApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void HRBJackpotCoinApplication::requestInitialize()
{
...
void HRBJackpotCoinApplication::requestShutdown()
{
...
void HRBJackpotCoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // hrbjackpotcoin: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &HRBJackpotCoinGUI::handlePaymentRequest);
            connect(window, &HRBJackpotCoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void HRBJackpotCoinApplication::handleRunawayException(const QString &message)
{
...
void HRBJackpotCoinApplication::handleNonFatalException(const QString& message)
{
...
WId HRBJackpotCoinApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool HRBJackpotCoinApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", HRBJackpotCoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(hrbjackpotcoin);
    Q_INIT_RESOURCE(hrbjackpotcoin_locale);
    ...
    /// 6-7. Parse hrbjackpotcoin.conf, determine network, switch to network specific
    ...
    // hrbjackpotcoin: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
...
#ifdef ENABLE_WALLET
void HRBJackpotCoinApplication::quickSetupWallets()
{
    // Minimal stub so the project links. We’ll replace this with the real
    // one-click “Mining + Spend” setup after we’re building cleanly.
    if (window) {
        // Call the existing “Create Wallet…” flow for now.
        QMetaObject::invokeMethod(window, "createWallet", Qt::QueuedConnection);
    }
}
#endif
