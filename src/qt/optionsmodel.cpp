...
/** Call node.updateRwSetting() with HRBJackpotCoin 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because HRBJackpotCoin 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/hrbjackpotcoin/hrbjackpotcoin/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because hrbjackpotcoin 22.x will not try to read these.
...
//! Convert enabled/size values to hrbjackpotcoin -prune setting.
...
//! Get pruning enabled value to show in GUI from hrbjackpotcoin -prune setting.
...
//! Get pruning size value to show in GUI from hrbjackpotcoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayHRBJackpotCoinUnit")) {
        settings.setValue("DisplayHRBJackpotCoinUnit", QVariant::fromValue(HRBJackpotCoinUnit::HRB));
    }
    QVariant unit = settings.value("DisplayHRBJackpotCoinUnit");
    if (unit.canConvert<HRBJackpotCoinUnit>()) {
        m_display_hrbjackpotcoin_unit = unit.value<HRBJackpotCoinUnit>();
    } else {
        m_display_hrbjackpotcoin_unit = HRBJackpotCoinUnit::HRB;
        settings.setValue("DisplayHRBJackpotCoinUnit", QVariant::fromValue(m_display_hrbjackpotcoin_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if hrbjackpotcoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_hrbjackpotcoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<HRBJackpotCoinUnit>() == m_display_hrbjackpotcoin_unit) return;
    m_display_hrbjackpotcoin_unit = new_unit.value<HRBJackpotCoinUnit>();
    QSettings settings;
    settings.setValue("DisplayHRBJackpotCoinUnit", QVariant::fromValue(m_display_hrbjackpotcoin_unit));
    Q_EMIT displayUnitChanged(m_display_hrbjackpotcoin_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/hrbjackpotcoin/hrbjackpotcoin/pull/8273
    ...
    // (https://github.com/hrbjackpotcoin-core/gui/issues/567).
