...
#include <qt/hrbjackpotcoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a HRBJackpotCoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new HRBJackpotCoinAddressEntryValidator(parent));
    widget->setCheckValidator(new HRBJackpotCoinAddressCheckValidator(parent));
}
...
bool parseHRBJackpotCoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no hrbjackpotcoin: URI
    if(!uri.isValid() || uri.scheme() != QString("hrbjackpotcoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!HRBJackpotCoinUnits::parse(HRBJackpotCoinUnit::HRB, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseHRBJackpotCoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseHRBJackpotCoinURI(uriInstance, out);
}

QString formatHRBJackpotCoinURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("hrbjackpotcoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(HRBJackpotCoinUnits::format(HRBJackpotCoinUnit::HRB, info.amount, false, HRBJackpotCoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openHRBJackpotCoinConf()
{
    ...
    /* Open hrbjackpotcoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == ChainType::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "HRBJackpotCoin.lnk";
    if (chain == ChainType::TESTNET) // Remove this special case when testnet CBaseChainParams::DataDir() is incremented to "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "HRBJackpotCoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("HRBJackpotCoin (%s).lnk", ChainTypeToString(chain)));
}

bool GetStartOnSystemStartup()
{
    // check for HRBJackpotCoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    ChainType chain = gArgs.GetChainType();
    if (chain == ChainType::MAIN)
        return GetAutostartDir() / "hrbjackpotcoin.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("hrbjackpotcoin-%s.desktop", ChainTypeToString(chain)));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        ChainType chain = gArgs.GetChainType();
        // Write a hrbjackpotcoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == ChainType::MAIN)
            optionFile << "Name=HRBJackpotCoin\n";
        else
            optionFile << strprintf("Name=HRBJackpotCoin (%s)\n", ChainTypeToString(chain));
