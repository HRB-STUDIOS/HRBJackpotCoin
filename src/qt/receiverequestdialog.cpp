...
#include <config/hrbjackpotcoin-config.h> /* for USE_QRCODE */
#endif
...
void ReceiveRequestDialog::setInfo(const SendCoinsRecipient &_info)
{
    ...
    QString uri = GUIUtil::formatHRBJackpotCoinURI(info);
...
{
    if (!model) return;
    ui->amount_content->setText(HRBJackpotCoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), info.amount));
}

void ReceiveRequestDialog::on_btnCopyURI_clicked()
{
    GUIUtil::setClipboard(GUIUtil::formatHRBJackpotCoinURI(info));
}
