...
// HRB Jackpot Coin custom includes for wallet addresses and base types
...
#include <tuple>
#include <utility>
#include <node/context.h> // for ChainstateManager

// HRB Jackpot Coin Jackpot Wallets and Seed System (raw scriptPubKey values reconstructed properly)
static const CScript MINI_JACKPOT_SCRIPT   = CScript() << OP_0 << ParseHex("abcdef1234567890abcdef1234567890abcdef");
static const CScript MINOR_JACKPOT_SCRIPT  = CScript() << OP_0 << ParseHex("abcdef1234567890abcdef1234567890abcdea");
static const CScript MAJOR_JACKPOT_SCRIPT  = CScript() << OP_0 << ParseHex("abcdef1234567890abcdef1234567890abcdeb");
static const CScript GRAND_JACKPOT_SCRIPT  = CScript() << OP_0 << ParseHex("abcdef1234567890abcdef1234567890abcdec");
static const CScript SEED_JACKPOT_SCRIPT   = CScript() << OP_0 << ParseHex("abcdef1234567890abcdef1234567890abcddd");
static const CScript FOUNDER_REPAY_SCRIPT  = CScript() << OP_0 << ParseHex("abcdef1234567890abcdef1234567890abcdee");

static constexpr CAmount JACKPOT_TAX_FEE = 50000; // 0.0005 HRB per tx
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if (!ancestors) {
        ...
        // this, see https://lists.linuxfoundation.org/pipermail/hrbjackpotcoin-dev/2018-November/016518.html
...
BlockReward GetBlockSubsidy(int nHeight, const Consensus::Params& params)
{
    CAmount baseReward;

    if (nHeight < 1051200)
        baseReward = 5000000000;         // 50 HRB
    else if (nHeight < 1576800)
        baseReward = 2500000000;         // 25 HRB
    else if (nHeight < 1839600)
        baseReward = 1250000000;         // 12.5 HRB
    else if (nHeight < 1971000)
        baseReward = 625000000;          // 6.25 HRB
    else {
        int halvings = (nHeight - 1971000) / 210000;
        baseReward = 312500000;          // 3.125 HRB
        baseReward >>= halvings;
    }

    // Determine if Golden or Diamond bonus applies
    CAmount multiplier = 1;
    if (nHeight % 7777 == 0)
        multiplier = 5; // Diamond block
    else if (nHeight % 777 == 0)
        multiplier = 2; // Golden block

    CAmount totalReward = baseReward * multiplier;

    // Apply fixed 0.25% fee
    CAmount fee = totalReward / 400; // 0.25%
    CAmount founderFee = fee / 10;   // 10% to founder
    CAmount jackpotFee = fee - founderFee;
    CAmount minerReward = totalReward - fee;

    // Split jackpotFee evenly across 4 jackpots
    CAmount perJackpot = jackpotFee / 4;

    BlockReward reward{
        .miner_reward          = minerReward,
        .founder_reimbursement = founderFee,
        .mini_jackpot          = perJackpot,
        .minor_jackpot         = perJackpot,
        .major_jackpot         = perJackpot,
        .grand_jackpot         = jackpotFee - 3 * perJackpot // handle rounding remainder
    };

    // Sanity check: all parts must sum to total
    assert(
        reward.miner_reward +
        reward.founder_reimbursement +
        reward.mini_jackpot +
        reward.minor_jackpot +
        reward.major_jackpot +
        reward.grand_jackpot == totalReward
    );
...
DisconnectResult Chainstate::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view)
{
    ...
    // See https://github.com/hrbjackpotcoin/hrbjackpotcoin/issues/22596 for additional information.
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // --- HRB Jackpot Coin: Fee Deduction & Accumulation Start ---
    const Consensus::Params& consensusParams = params.GetConsensus();

    BlockReward reward = GetBlockSubsidy(pindex->nHeight, consensusParams);

    // Accumulate jackpot fees (per-block, they are deducted from miner reward)
    pindex->nJackpotMini   = (pindex->pprev ? pindex->pprev->nJackpotMini   : 0) + reward.mini_jackpot;
    pindex->nJackpotMinor  = (pindex->pprev ? pindex->pprev->nJackpotMinor  : 0) + reward.minor_jackpot;
    pindex->nJackpotMajor  = (pindex->pprev ? pindex->pprev->nJackpotMajor  : 0) + reward.major_jackpot;
    pindex->nJackpotGrand  = (pindex->pprev ? pindex->pprev->nJackpotGrand  : 0) + reward.grand_jackpot;
    pindex->nFounderFeeAcc = (pindex->pprev ? pindex->pprev->nFounderFeeAcc : 0) + reward.founder_reimbursement;

    LogPrintf("Jackpot Accumulation - Height: %d | Mini: %d | Minor: %d | Major: %d | Grand: %d | Founder Acc: %d\n",
        pindex->nHeight,
        pindex->nJackpotMini,
        pindex->nJackpotMinor,
        pindex->nJackpotMajor,
        pindex->nJackpotGrand,
        pindex->nFounderFeeAcc);

    // --- End HRB Jackpot Coin Fee Accumulation ---
    ...
    // Start HRB Jackpot Fee & Payout Logic
    CAmount jackpot_fee_total = 0;
    for (const CTransactionRef& tx : block.vtx) {
        if (tx->IsCoinBase()) continue; // Skip coinbase transaction for fee calculation
        for (const CTxOut& out : tx->vout) {
            // Cap the fee at JACKPOT_TAX_FEE or 1% of output value, whichever is smaller
            CAmount fee = std::min(JACKPOT_TAX_FEE, out.nValue / 100);
            jackpot_fee_total += fee;
        }
    }

    if (jackpot_fee_total > 0) {
        LogPrint(BCLog::VALIDATION, "Jackpot Tax Applied: %d jackHRB from block %s\n", jackpot_fee_total, pindex->GetBlockHash().ToString());

        // --- Jackpot Payout and Reimbursement System ---
        // This section defines the conceptual payout rules and logging.
        // ACTUAL ON-CHAIN TRANSACTION CREATION AND UTXO MANAGEMENT IS HIGHLY COMPLEX
        // AND REQUIRES SIGNIFICANT ADDITIONAL DEVELOPMENT BEYOND A SIMPLE INJECTION.
        // It would typically involve modifying the coinbase transaction or creating new
        // output transactions, which is consensus-critical and requires careful handling
        // of script construction, transaction signing, and UTXO set updates.

        // Determine the block for randomness (e.g., 10 blocks back)
        const int K_BLOCKS_BACK = 10;
        const CBlockIndex* random_block_index = nullptr;
        if (pindex->nHeight > K_BLOCKS_BACK) {
            random_block_index = pindex->GetAncestor(pindex->nHeight - K_BLOCKS_BACK);
        }

        uint256 randomness_hash;
        if (random_block_index) {
            randomness_hash = random_block_index->GetBlockHash();
            LogPrint(BCLog::VALIDATION, "Jackpot randomness derived from block %d hash: %s\n", random_block_index->nHeight, randomness_hash.ToString());
        } else {
            // Fallback for early blocks or if ancestor not found (shouldn't happen with assert)
            randomness_hash = pindex->GetBlockHash();
            LogPrint(BCLog::VALIDATION, "Jackpot randomness derived from current block %d hash: %s (early block)\n", pindex->nHeight, randomness_hash.ToString());
        }

        // --- Jackpot Win Detection ---
        bool golden_block_win = (pindex->nHeight % 777 == 0 && pindex->nHeight % 7777 != 0);
        bool diamond_block_win = (pindex->nHeight % 7777 == 0);

        std::string jackpot_type = "None";
        CAmount current_jackpot_reward = 0; // This would be the amount to pay out for a win

        if (diamond_block_win) {
            jackpot_type = "Diamond";
            // In a full system, this would pull from a progressive pool or be a fixed amount
            current_jackpot_reward = jackpot_fee_total * 5; // Conceptual: 5x the collected fees for this block
            LogPrint(BCLog::VALIDATION, "DIAMOND BLOCK JACKPOT WIN at height %d! Conceptual reward: %d jackHRB\n", pindex->nHeight, current_jackpot_reward);
        } else if (golden_block_win) {
            jackpot_type = "Golden";
            current_jackpot_reward = jackpot_fee_total * 2; // Conceptual: 2x the collected fees for this block
            LogPrint(BCLog::VALIDATION, "GOLDEN BLOCK JACKPOT WIN at height %d! Conceptual reward: %d jackHRB\n", pindex->nHeight, current_jackpot_reward);
        } else {
            // Progressive Jackpots (Mini, Minor, Major, Grand) - Probabilistic Hash Match
            // This is a simplified example. Real systems use more robust, verifiable randomness.
            std::string hash_str = randomness_hash.GetHex();

            if (hash_str.rfind("000000", 0) == 0) { // Starts with 6 zeros
                jackpot_type = "Grand Progressive";
                // Conceptual: Pull from a large, accumulated Grand Jackpot pool
                current_jackpot_reward = jackpot_fee_total * 10; // Example: 10x collected fees
                LogPrint(BCLog::VALIDATION, "GRAND JACKPOT TRIGGERED by hash %s! Conceptual reward: %d jackHRB\n", hash_str, current_jackpot_reward);
            } else if (hash_str.rfind("0000", 0) == 0) { // Starts with 4 zeros
                jackpot_type = "Major Progressive";
                current_jackpot_reward = jackpot_fee_total * 5; // Example: 5x collected fees
                LogPrint(BCLog::VALIDATION, "MAJOR JACKPOT TRIGGERED by hash %s! Conceptual reward: %d jackHRB\n", hash_str, current_jackpot_reward);
            } else if (hash_str.rfind("00", 0) == 0) { // Starts with 2 zeros
                jackpot_type = "Minor Progressive";
                current_jackpot_reward = jackpot_fee_total * 2; // Example: 2x collected fees
                LogPrint(BCLog::VALIDATION, "MINI JACKPOT TRIGGERED by hash %s! Conceptual reward: %d jackHRB\n", hash_str, current_jackpot_reward);
            } else if (hash_str.rfind("0", 0) == 0) { // Starts with 1 zero
                jackpot_type = "Mini Progressive";
                current_jackpot_reward = jackpot_fee_total * 1.5; // Example: 1.5x collected fees
                LogPrint(BCLog::VALIDATION, "MINI JACKPOT TRIGGERED by hash %s! Conceptual reward: %d jackHRB\n", hash_str, current_jackpot_reward);
            }
        }

        // --- Conceptual Payout Routing ---
        // This is where the actual on-chain distribution would be built.
        // For a real system, you would need to:
        // 1. Manage actual jackpot pools (e.g., UTXOs held by specific addresses).
        // 2. Create new transactions (or modify the coinbase transaction) to send funds.
        // 3. Ensure these transactions are valid and follow consensus rules.
        // 4. Handle founder repayment logic (e.g., track a total repayment amount).

        // For now, we'll log the conceptual distribution of the collected fees
        // and any triggered jackpot rewards.

        LogPrint(BCLog::VALIDATION, "Conceptual Jackpot Fee Distribution for block %d:\n", pindex->nHeight);
        LogPrint(BCLog::VALIDATION, "  Total Collected Fees: %d jackHRB\n", jackpot_fee_total);
        if (current_jackpot_reward > 0) {
            // This is a placeholder for sending the "reward" amount to the winning miner.
            // In a real system, this would likely be an extra output in the coinbase transaction.
            LogPrint(BCLog::VALIDATION, "  %s Jackpot Reward (Conceptual to winning miner): %d jackHRB\n", jackpot_type, current_jackpot_reward);
        }

        // Conceptual routing of the collected fees (jackpot_fee_total)
        // to the various wallets. This is separate from the 'current_jackpot_reward'.
        // This would typically involve creating new outputs in the coinbase transaction
        // or separate transactions.

        // Example: If founder repayment is active, redirect a portion.
        // This would require a persistent founder repayment state (e.g., in chainstate DB).
        /*
        if (g_founder_repayment_active) { // Conceptual global flag
            CAmount founder_share = jackpot_fee_total / 2; // Example: 50% to founder
            LogPrint(BCLog::VALIDATION, "  Conceptual Founder Repayment: %d jackHRB to %s\n", founder_share, EncodeDestination(FOUNDER_REPAY_WALLET));
            // remaining_fees -= founder_share;
        }
        */

        // Conceptual distribution to progressive jackpot pools/seed wallet
        // This would involve crediting internal pools or creating new outputs.
        // The percentages below are illustrative.
        LogPrint(BCLog::VALIDATION, "  Conceptual routing of collected fees to pools:\n");
        LogPrint(BCLog::VALIDATION, "    To SEED_JACKPOT_WALLET: %d jackHRB (e.g., 20%% of collected fees)\n", jackpot_fee_total / 5);
        LogPrint(BCLog::VALIDATION, "    To MINI_JACKPOT_WALLET pool: %d jackHRB (e.g., 15%%)\n", jackpot_fee_total * 3 / 20);
        LogPrint(BCLog::VALIDATION, "    To MINOR_JACKPOT_WALLET pool: %d jackHRB (e.g., 10%%)\n", jackpot_fee_total / 10);
        LogPrint(BCLog::VALIDATION, "    To MAJOR_JACKPOT_WALLET pool: %d jackHRB (e.g., 5%%)\n", jackpot_fee_total / 20);
        LogPrint(BCLog::VALIDATION, "    To GRAND_JACKPOT_WALLET pool: %d jackHRB (e.g., 5%%)\n", jackpot_fee_total / 20);
        // Remaining might go to miner or other network funds.

        LogPrint(BCLog::VALIDATION, "--- End Jackpot Payout and Reimbursement System ---\n");
    }
    // End HRB Jackpot Fee & Payout Logic
    CAmount nFees = 0;
    int nInputs = 0;
    ...
    CAmount newCoins = GetBlockSubsidy(pindex->nHeight, params.GetConsensus());
    if (pindex->pprev) {
        pindex->nMoneySupply = pindex->pprev->nMoneySupply + newCoins;
    } else {
        pindex->nMoneySupply = newCoins; // Genesis block
    }
...
bool ChainstateManager::ProcessNewBlock(const std::shared_ptr<const CBlock>& block, bool force_processing, bool min_pow_checked, bool* new_block)
{
...

    {
        ...
        // https://lists.linuxfoundation.org/pipermail/hrbjackpotcoin-dev/2019-February/016697.html.  Because CheckBlock() is
