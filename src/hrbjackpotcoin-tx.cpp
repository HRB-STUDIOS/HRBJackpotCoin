...
static void SetupHRBJackpotCoinTxArgs(ArgsManager &argsman)
{
...
{
    SetupHRBJackpotCoinTxArgs(gArgs);
    std::string error;
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " hrbjackpotcoin-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.IsArgSet("-version")) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "Usage:  hrbjackpotcoin-tx [options] <hex-tx> [commands]  Update hex-encoded hrbjackpotcoin transaction\n"
                "or:     hrbjackpotcoin-tx [options] -create [commands]   Create hex-encoded hrbjackpotcoin transaction\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded hrbjackpotcoin transaction
