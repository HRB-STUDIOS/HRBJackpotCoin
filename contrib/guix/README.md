# Bootstrappable HRBJackpotCoin Core Builds

This directory contains the files necessary to perform bootstrappable HRBJackpotCoin
Core builds.
...
To build HRBJackpotCoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      HRBJackpotCoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/hrbjackpotcoin-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/hrbjackpotcoin-core/guix.sigs).
