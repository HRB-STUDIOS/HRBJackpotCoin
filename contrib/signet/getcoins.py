#!/usr/bin/env python3
# Copyright (c) 2020-2022 The HRBJackpotCoin Core developers
...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing hrbjackpotcoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='hrbjackpotcoin-cli', help='hrbjackpotcoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='HRBJackpotCoin address to which the faucet should send')
...
parser.add_argument('hrbjackpotcoin_cli_args', nargs='*', help='Arguments to pass on to hrbjackpotcoin-cli (default: -signet)')

args = parser.parse_args()

if args.hrbjackpotcoin_cli_args == []:
    args.hrbjackpotcoin_cli_args = ['-signet']


def hrbjackpotcoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.hrbjackpotcoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = hrbjackpotcoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = hrbjackpotcoin_cli(['getnewaddress', 'faucet', 'bech32'])
