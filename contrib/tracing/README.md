...
Tracing (USDT) support for HRBJackpotCoin Core on Linux using. For more information on
USDT support in HRBJackpotCoin Core see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `hrbjackpotcoind` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `hrbjackpotcoind` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[HRBJackpotCoin Core USDT documentation] on how to list available tracepoints in your
`hrbjackpotcoind` binary.

[HRBJackpotCoin Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py ./src/hrbjackpotcoind
```
...
HRBJackpotCoin P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py ./src/hrbjackpotcoind
```
...
In a different terminal, starting HRBJackpotCoin Core in SigNet mode and with
re-indexing enabled.

```
$ ./src/hrbjackpotcoind -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py ./src/hrbjackpotcoind
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py ./src/hrbjackpotcoind
```
