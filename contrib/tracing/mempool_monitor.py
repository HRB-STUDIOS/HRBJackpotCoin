...
""" Example logging HRBJackpotCoin Core mempool events using the mempool:added,
...
def main(hrbjackpotcoind_path):
    hrbjackpotcoind_with_usdts = USDT(path=str(hrbjackpotcoind_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    hrbjackpotcoind_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    hrbjackpotcoind_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    hrbjackpotcoind_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    hrbjackpotcoind_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[hrbjackpotcoind_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/hrbjackpotcoind")
        exit(1)
