#!/usr/bin/env python3
# Copyright (c) 2014-2022 The HRBJackpotCoin Core developers
...
TMPDIR_PREFIX = "hrbjackpotcoin_func_test_"
...
class HRBJackpotCoinTestMetaClass(type):
    """Metaclass for HRBJackpotCoinTestFramework.

    Ensures that any attempt to register a subclass of `HRBJackpotCoinTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'HRBJackpotCoinTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("HRBJackpotCoinTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("HRBJackpotCoinTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class HRBJackpotCoinTestFramework(metaclass=HRBJackpotCoinTestMetaClass):
    """Base class for a hrbjackpotcoin test script.

    Individual hrbjackpotcoin test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave hrbjackpotcoinds and test.* datadir on exit or error")
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop hrbjackpotcoinds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use hrbjackpotcoin-cli instead of RPC for all commands")
    ...
    def set_binary_paths(self):
        ...
        binaries = {
            "hrbjackpotcoind": ("hrbjackpotcoind", "HRBJACKPOTCOIND"),
            "hrbjackpotcoin-cli": ("hrbjackpotcoincli", "HRBJACKPOTCOINCLI"),
            "hrbjackpotcoin-util": ("hrbjackpotcoinutil", "HRBJACKPOTCOINUTIL"),
            "hrbjackpotcoin-wallet": ("hrbjackpotcoinwallet", "HRBJACKPOTCOINWALLET"),
        }
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: hrbjackpotcoinds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("HRBJackpotCoinRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'hrbjackpotcoind', self.options.hrbjackpotcoind) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'hrbjackpotcoin-cli', self.options.hrbjackpotcoincli) for v in versions]
        ...
        for i in range(num_nodes):
            ...
            test_node_i = TestNode(
                ...
                hrbjackpotcoind=binary[i],
                hrbjackpotcoin_cli=binary_cli[i],
                version=versions[i],
    ...
    def start_nodes(self, extra_args=None, *args, **kwargs):
        """Start multiple hrbjackpotcoinds"""
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a hrbjackpotcoind test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple hrbjackpotcoind test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as hrbjackpotcoind's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    hrbjackpotcoind=self.options.hrbjackpotcoind,
                    hrbjackpotcoin_cli=self.options.hrbjackpotcoincli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in hrbjackpotcoin.conf
    ...
    def skip_if_no_hrbjackpotcoind_tracepoints(self):
        """Skip the running test if hrbjackpotcoind has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("hrbjackpotcoind has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_hrbjackpotcoind_zmq(self):
        """Skip the running test if hrbjackpotcoind has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("hrbjackpotcoind has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if hrbjackpotcoin-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("hrbjackpotcoin-wallet has not been compiled")

    def skip_if_no_hrbjackpotcoin_util(self):
        """Skip the running test if hrbjackpotcoin-util has not been compiled."""
        if not self.is_hrbjackpotcoin_util_compiled():
            raise SkipTest("hrbjackpotcoin-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if hrbjackpotcoin-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("hrbjackpotcoin-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether hrbjackpotcoin-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether hrbjackpotcoin-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_hrbjackpotcoin_util_compiled(self):
        """Checks whether hrbjackpotcoin-util was compiled."""
        return self.config["components"].getboolean("ENABLE_HRBJACKPOTCOIN_UTIL")
