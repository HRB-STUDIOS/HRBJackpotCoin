...
`test/functional/test_framework/test_framework.py:HRBJackpotCoinTestFramework.main`.
...
BASE_SCRIPTS = [
    ...
    'interface_hrbjackpotcoin_cli.py --legacy-wallet',
    'interface_hrbjackpotcoin_cli.py --descriptors',
...
def main():
    ...
    enable_hrbjackpotcoind = config["components"].getboolean("ENABLE_HRBJACKPOTCOIND")

    if not enable_hrbjackpotcoind:
...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control):
    ...
    # Warn if hrbjackpotcoind is already running
    try:
        # pgrep exits with code zero when one or more matching processes found
        if subprocess.run(["pgrep", "-x", "hrbjackpotcoind"], stdout=subprocess.DEVNULL).returncode == 0:
            print("%sWARNING!%s There is already a hrbjackpotcoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    except OSError:
        # pgrep not supported
        pass
    ...
    # a hard link or a copy on any platform. See https://github.com/hrbjackpotcoin/hrbjackpotcoin/pull/27561.
...
class RPCCoverage():
    ...
    commands per `hrbjackpotcoin-cli help` (`rpc_interface.txt`).
