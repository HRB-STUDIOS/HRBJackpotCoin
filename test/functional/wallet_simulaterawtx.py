#!/usr/bin/env python3
# Copyright (c) 2021-2022 The HRBJackpotCoin Core developers
...
class SimulateTxTest(HRBJackpotCoinTestFramework):
    ...
    def run_test(self):
        ...
        hrbjackpotcoin_fee = Decimal(funding["fee"])

        # w0 sees fee + 5 hrb decrease, w2 sees + 5 hrb
        assert_approx(w0.simulaterawtransaction([tx1])["balance_change"], -(Decimal("5") + hrbjackpotcoin_fee))
        ...
        #       wallet                  tx3                             tx4
        #       w0                      -5 - hrbjackpotcoin_fee + 4.9999       -5 - hrbjackpotcoin_fee
        #       w1                      0                               +4.9999
        assert_approx(w0.simulaterawtransaction([tx1, tx3])["balance_change"], -Decimal("5") - hrbjackpotcoin_fee + Decimal("4.9999"))
        assert_approx(w1.simulaterawtransaction([tx1, tx3])["balance_change"], 0)
        assert_approx(w0.simulaterawtransaction([tx1, tx4])["balance_change"], -Decimal("5") - hrbjackpotcoin_fee)
        ...
        tx2 = funding["hex"]
        hrbjackpotcoin_fee2 = Decimal(funding["fee"])
        assert_approx(w0.simulaterawtransaction([tx2])["balance_change"], -(Decimal("10") + hrbjackpotcoin_fee2))
