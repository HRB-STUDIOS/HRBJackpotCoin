...
Below are some notes on how to build HRBJackpotCoin Core for Windows.

The options known to work for building HRBJackpotCoin Core on Windows are:
...
Note that for WSL the HRBJackpotCoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/hrbjackpotcoin, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\hrbjackpotcoin`, for example:

    make install DESTDIR=/mnt/c/workspace/hrbjackpotcoin
