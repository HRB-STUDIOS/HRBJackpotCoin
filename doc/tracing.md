# User-space, Statically Defined Tracing (USDT) for HRBJackpotCoin Core

HRBJackpotCoin Core includes statically defined tracepoints to allow for more
...
======================
                ...
                ┌──────────────────┐            ┌──────────────┐
                │ tracing script   │            │ hrbjackpotcoind     │
                │==================│      2.    │==============│
                │  eBPF  │ tracing │      hooks │              │
...
**Note**: HRBJackpotCoin Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to HRBJackpotCoin Core
...
Multiple tools can list the available tracepoints in a `hrbjackpotcoind` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in HRBJackpotCoin Core, use `info probes` in `gdb`:

```
$ gdb ./src/hrbjackpotcoind
…
(gdb) info probes
Type Provider   Name             Where              Semaphore Object
stap net        inbound_message  0x000000000014419e /src/hrbjackpotcoind
stap net        outbound_message 0x0000000000107c05 /src/hrbjackpotcoind
stap validation block_connected  0x00000000002fb10c /src/hrbjackpotcoind
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in HRBJackpotCoin Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/hrbjackpotcoind | grep NT_STAPSDT -A 4 -B 2
...
```
$ tplist -l ./src/hrbjackpotcoind -v
