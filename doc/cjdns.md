# CJDNS support in HRBJackpotCoin Core

It is possible to run HRBJackpotCoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of HRBJackpotCoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the HRBJackpotCoin network and individual nodes.
...
before it will work with your HRBJackpotCoin Core node. This is described in steps
...
## Run HRBJackpotCoin Core with CJDNS

Once you are connected to the CJDNS network, the following HRBJackpotCoin Core
...
When enabled, this option tells HRBJackpotCoin Core that it is running in an
...
IPv6 local network. This helps HRBJackpotCoin Core perform better address management:
...
CJDNS support was added to HRBJackpotCoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `hrbjackpotcoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/hrbjackpotcoin/hrbjackpotcoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in HRBJackpotCoin Core

There are several ways to see your CJDNS address in HRBJackpotCoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `hrbjackpotcoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `hrbjackpotcoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `hrbjackpotcoin-cli help getnodeaddresses` for details.
