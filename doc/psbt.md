# PSBT Howto for HRBJackpotCoin Core

Since HRBJackpotCoin Core 0.17, an RPC interface exists for Partially Signed HRBJackpotCoin
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/hrbjackpotcoin/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for HRBJackpotCoin transactions that are not fully signed
...
#### Multisig with multiple HRBJackpotCoin Core instances
...
HRBJackpotCoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `hrbjackpotcoin-cli` is used.
...
Later, when *V* hrb has been received on *Amulti*, and Bob and Carol want to
