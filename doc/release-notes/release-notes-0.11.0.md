HRBJackpotCoin Core version 0.11.0 is now available from:

  <https://hrbjackpotcoin.org/bin/hrbjackpotcoin-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer hrb/kB fee to be
...
For example, add the following to `hrbjackpotcoin.conf`:
...
related to the blockchain in the hrbjackpotcoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows HRBJackpotCoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that HRBJackpotCoin Core will 
...
For an example script see [hrbjackpotcoin-submittx](https://github.com/laanwj/hrbjackpotcoin-submittx).
...
- #5334 `cf87536` libhrbjackpotcoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new hrbjackpotcoin-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only HRBJackpotCoinAmountField
- #5753 `0f44672` Add hrbjackpotcoin logo to about screen
...
- #5809 `46bfbe7` Add hrbjackpotcoin-cli man page
...
- #5849 `41259ca` contrib/init/hrbjackpotcoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename HRBJackpotCoin-Qt.app to HRBJackpotCoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for hrbjackpotcoin-tx)
