HRBJackpotCoin Core version 0.18.0 is now available from:

  <https://hrbjackpotcoincore.org/bin/hrbjackpotcoin-core-0.18.0/>
...
`/Applications/HRBJackpotCoin-Qt` (on Mac) or `hrbjackpotcoind`/`hrbjackpotcoin-qt` (on
Linux).
...
recommended to use HRBJackpotCoin Core on unsupported systems.

HRBJackpotCoin Core should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, HRBJackpotCoin Core does not yet change appearance when
...
`contrib/init/hrbjackpotcoind.service`, it has been changed to use
`/var/lib/hrbjackpotcoind` as the data directory instead of
`~hrbjackpotcoin/.hrbjackpotcoin`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/hrbjackpotcoind` will
exist has enough space (check using `df -h /var/lib/hrbjackpotcoind`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/hrbjackpotcoin/hrbjackpotcoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://hrbjackpotcoincore.org/en/contact/) the HRBJackpotCoin Core
...
The systemd init file (`contrib/init/hrbjackpotcoind.service`) has been changed
to use `/var/lib/hrbjackpotcoind` as the data directory instead of
`~hrbjackpotcoin/.hrbjackpotcoin`. This change makes HRBJackpotCoin Core more consistent with
...
`datadir` option in `/etc/hrbjackpotcoin/hrbjackpotcoin.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/hrbjackpotcoin/hrbjackpotcoin.conf`.


Documentation
-------------

- A new short [document](https://github.com/hrbjackpotcoin/hrbjackpotcoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/hrbjackpotcoin/hrbjackpotcoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/hrbjackpotcoin/hrbjackpotcoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/hrbjackpotcoin/hrbjackpotcoin/blob/master/doc/hrbjackpotcoin-conf.md)
  about the `hrbjackpotcoin.conf` file describes how to use it to configure
  HRBJackpotCoin Core.

- A new document introduces HRBJackpotCoin Core's BIP174 [Partially-Signed
  HRBJackpotCoin Transactions
  (PSBT)](https://github.com/hrbjackpotcoin/hrbjackpotcoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/hrbjackpotcoin/hrbjackpotcoin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent HRBJackpotCoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  HRBJackpotCoin Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/hrbjackpotcoin/hrbjackpotcoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/hrbjackpotcoin/hrbjackpotcoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when HRBJackpotCoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of HRBJackpotCoin Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/hrbjackpotcoin/hrbjackpotcoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/hrbjackpotcoin/hrbjackpotcoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart hrbjackpotcoind with the `-deprecatedrpc=generate`
...
- A new `hrbjackpotcoin-wallet` tool is now distributed alongside HRBJackpotCoin
...
This section describes planned changes to HRBJackpotCoin Core that may affect
other HRBJackpotCoin software and services.

- Since version 0.16.0, HRBJackpotCoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with HRBJackpotCoin Core 0.20 (expected about
  a year after 0.18), HRBJackpotCoin Core will default to native segwit
  ...
  to bech32 addresses, and if the HRBJackpotCoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in HRBJackpotCoin Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any HRBJackpotCoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, HRBJackpotCoin Core
...
- A sub-project of HRBJackpotCoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with HRBJackpotCoin Core.  See their [project
  page](https://github.com/hrbjackpotcoin-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to HRBJackpotCoin Core's own implementation, although entropy
  gathered by HRBJackpotCoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves HRBJackpotCoin Core a
...
- On macOS, HRBJackpotCoin Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `hrbjackpotcoin_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libhrbjackpotcoin_server.a` (jnewbery)
...
- #13926 [Tools] hrbjackpotcoin-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like hrbjackpotcoind (MarcoFalke)
...
- #14521 qt, docs: Fix `hrbjackpotcoin-qt -version` output formatting (hebasto)
...
- #11625 Add HRBJackpotCoinApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libhrbjackpotcoin_server` (laanwj)
...
- #15154 Configure: hrbjackpotcoin-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/hrbjackpotcoin-wallet (Sjors)
...
- #11526 Visual Studio build configuration for HRBJackpotCoin Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libhrbjackpotcoin_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in hrbjackpotcoin-util-test.py (isghe)
...
- #12775 Integration of property based testing into HRBJackpotCoin Core (Christewart)
...
- #12246 Bugfix: Only run hrbjackpotcoin-tx tests when hrbjackpotcoin-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_hrbjackpotcoin` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_hrbjackpotcoin` (MarcoFalke)
...
- #14186 hrbjackpotcoin-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_hrbjackpotcoin.conf` message on startup if conf file exists (leishman)
...
- #12255 Update hrbjackpotcoin.service to conform to init.md (dongcarl)
...
- #14497 Add doc/hrbjackpotcoin-conf.md (hebasto)
...
- #15354 Add missing `hrbjackpotcoin-wallet` tool manpages (MarcoFalke)
...
- #15765 correct hrbjackpotcoinconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support hrbjackpotcoin folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/hrbjackpotcoin/).
